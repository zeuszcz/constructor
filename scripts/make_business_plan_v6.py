...
usp_data = [
    ["#", "Что у нас", "Почему это важно клиенту", "У конкурентов"],
...
t = Table(usp_data, colWidths=[8*mm, 38*mm, 75*mm, 50*mm])
ts = std_table_style()
ts.add("BACKGROUND", (1, 1), (1, -1), NAVY_LIGHT)
ts.add("FONTNAME", (1, 1), (1, -1), BOLD)
ts.add("VALIGN", (0, 0), (-1, -1), "TOP")
ts.add("FONTSIZE", (0, 1), (-1, -1), 8.5)
t.setStyle(ts)
story.append(t)
story.append(Spacer(1, 6*mm))
...
primary = [
    ...
    ["Доход бизнеса", "ARPU 50–200 К ₽/мес выручки (это «средний» SMB, не микро, не средний)"],
    ...
    ["Триггер покупки", "Конкурент запустил сайт. Новый продукт. Сезонность (Q4). Выход на маркетплейсы."],
]
t = Table(primary, colWidths=[50*mm, 121*mm])
ts = std_table_style()
...
secondary = [
    ...
    ["Что для них важно", "API, white-label, скорость сборки (важнее эстетики), белая бухгалтерия"],
]
t = Table(secondary, colWidths=[50*mm, 121*mm])
ts = std_table_style()
ts.add("FONTNAME", (0, 1), (0, -1), BOLD)
ts.add("BACKGROUND", (0, 1), (0, -1), NAVY_LIGHT)
ts.add("VALIGN", (0, 0), (-1, -1), "TOP")
...
not_target = [
    ["Сегмент", "Почему НЕ наш", "Куда уходит"],
    ...
    ["Крупный B2B-enterprise (500+ чел.)", "Корпоративные ИТ-отделы, ТЗ на 100 стр.", "1С-Битрикс, Bitrix24 Ent"],
    ["Государственные / бюджет", "Тендеры, 44-ФЗ, спец. требования", "Госуслуги / РТ-Лабс"],
]
t = Table(not_target, colWidths=[55*mm, 66*mm, 50*mm])
ts = std_table_style()
...
story.append(Paragraph(
    "Из плана v5 убраны три узкие группы: «Голосовые роботы» (186 показов, мёртвая ниша), "
    "«152-ФЗ как реклама» (127 показов, не покупательский запрос), «Системы записи» (573 показов, "
    "слишком узкая ниша — для нас это feature, не main offering). Бюджет перераспределён "
    "на «Внедрение ИИ» (sharp rise) и новые сезонные запросы.",
    NOTE
))
...
groups = [
    ...
    ["A. Сайты под ключ", "7 192", "7 192", "100%", "Q4 рост", "Lite / Starter"],
    ...
    ["НОВОЕ: «Лучшая CRM 2026»", "—", "~1 200", "100%", "пик Q3", "Pro / Ent"],
    ["НОВОЕ: YandexGPT для бизнеса", "—", "~800", "100%", "растёт", "Pro / Ent"],
    ["ВСЕГО", "34 342", "~31 180", "91%", "—", "—"],
]
t = Table(groups, colWidths=[52*mm, 22*mm, 22*mm, 18*mm, 28*mm, 28*mm])
ts = std_table_style()
ts.add("BACKGROUND", (0, -1), (-1, -1), NAVY_LIGHT)
ts.add("FONTNAME", (0, -1), (-1, -1), BOLD)
...
gt_top = [
    ...
    ["1", "лучшая crm 2026", "100", "резкий скачок", "0", "Сезонный пик 3 квартал!"],
    ["2", "внедрение ии", "55", "очень резкий рост", "0", "+400–600% к прошлому году"],
    ["3", "yandexgpt для бизнеса", "45", "волатильный рост", "0", "Повторный рост в 2026"],
    ["4", "создание сайта под ключ", "39", "стабильная сезонность", "1 324", "4 квартал рост"],
    ["5", "интеграция битрикс24", "37", "стабильно", "2 549", "Эвергрин CRM"],
    ["6", "автоматизация бизнес процессов", "35", "растёт", "6 508", "Лидер Wordstat"],
    ["7", "разработка интернет магазина", "31", "стаб. сезонность", "1 817", "4 квартал пик"],
    ["8", "crm для бизнеса", "30", "плоский рост", "1 535", "Эвергрин B2B"],
    ["9", "автоматизация рутинных задач", "25", "стабильно", "0", "Подходит для SEO-контента"],
    ["10", "автоматизация продаж", "20", "стабильно", "3 140", "Стабильный B2B"],
]
...
principles = [
    ...
    ["1", "Каждый тариф — отдельный сегмент",
     "Lite → SMB-владелец с 1 сайтом. Starter → SMB с 3 проектами. Pro → магазин/SaaS. Enterprise → агентства."],
    ...
    ["5", "Free-tier с лимитами + 3-дн пробник Pro",
     "Free живёт вечно с ограничениями (поддомен, 1 проект). Pro — на 3 дня без карты."],
]
t = Table(principles, colWidths=[8*mm, 48*mm, 115*mm])
ts = std_table_style()
ts.add("VALIGN", (0, 0), (-1, -1), "TOP")
ts.add("FONTNAME", (1, 1), (1, -1), BOLD)
t.setStyle(ts)
...
evolution = [
    ["Период", "Lite", "Starter", "Pro", "Enterprise", "ARPU", "Логика"],
...
t = Table(evolution, colWidths=[28*mm, 16*mm, 20*mm, 16*mm, 22*mm, 22*mm, 47*mm])
ts = std_table_style()
ts.add("FONTNAME", (0, 1), (0, -1), BOLD)
ts.add("ALIGN", (1, 1), (-2, -1), "CENTER")
t.setStyle(ts)
...
hyp = [
    ["#", "Гипотеза", "Как проверим", "Что делаем если ошиблись"],
    ...
    ["H3", "Средний срок жизни клиента 12 мес",
     "Смотрим Cohort retention на 6-м месяце", "Если < 8 мес — снижаем цену или добавляем функции"],
    ["H4", "Pro/Enterprise возьмут только агентства",
     "Сегментируем покупателей Pro+", "Если SMB берёт Pro — упрощаем UI, убираем агентские функции"],
    ["H5", "Доп. услуги дадут 15% выручки",
     "Считаем revenue mix через 6 мес", "Если < 5% — убираем как distraction"],
]
t = Table(hyp, colWidths=[8*mm, 48*mm, 50*mm, 65*mm])
ts = std_table_style()
ts.add("VALIGN", (0, 0), (-1, -1), "TOP")
ts.add("FONTNAME", (1, 1), (1, -1), BOLD)
t.setStyle(ts)
...
struct = [
    ...
    ["Маркетинг и реклама", "Фиксированный бюджет",
     "100 К → 500 К ₽/мес. Растёт по сезонам (Q4 пик, Q3 подготовка)."],
    ["Расходы на пробник", "Доп. строка",
     "20 К → 90 К ₽/мес. Зависит от объёма пробников. Окупается 6.3× в первый месяц."],
    ["ИТОГО операционная маржа до маркетинга", "30%", "На каждом рубле остаётся 30 копеек."],
]
...
growth_assumptions = [
    ["Период", "Что происходит", "Платящих на конец периода"],
    ...
    ["M7-M12 (ноябрь 2026 - апрель 2027)", "Q4 рост + Q1 стабилизация. Партнёрки с AmoCRM.", "1 670"],
...
arpu_assumptions = [
    ["Период", "Средний доход с клиента", "Почему такой"],
    ...
    ["M25-M36 (зрелость 2)", "3 320–4 080 ₽/мес", "Стабилизация на зрелой смеси: 45% Lite + 25% Starter + 20% Pro + 10% Ent"],
]
...
sources = [
    ...
    ["3", "Годовые подписки со скидкой 50% (предзаказы)",
     "30 × 5 940 = 178 К ₽", "M2-M3"],
    ["4", "Барт­ер с фрилансерами (дизайнер за Pro-подписку)",
     "Экономия 60-80 К/мес", "M3-M9"],
    ["5", "Партнёрки с AmoCRM / Bitrix24 (15-20% revenue share)",
     "+50-150 К/мес дополн. выручки", "M6+"],
    ["6", "Yandex AI Studio Boost (фонд 500 млн руб)",
     "До 5 М ₽ (заявка)", "M2-M6"],
    ["7", "Кастом-разработка для Pro/Enterprise клиентов",
     "+50-200 К/мес (часы джунов)", "M6+"],
]
...
budget = [
    ["Группа", "v7 %", "v8 %", "Изменение", "Обоснование"],
    ...
    ["G. Внедрение ИИ (резкий рост)", "15%", "22%", "+7", "GT подтвердил +400–600%"],
    ["D. Чат-боты", "10%", "10%", "—", "100% IT"],
    ["B. AI-конструктор (узкий)", "1%", "0%", "−1", "Перераспределён"],
    ["НОВЫЕ GT-лендинги (CRM+ИИ)", "0%", "5%", "+5", "«Лучшая CRM 2026» + YandexGPT"],
    ["UBR: голос. роботы / 152-ФЗ / системы записи", "2%", "0%", "−2", "Убраны как нерелевантные"],
    ["Тестовый бюджет (новые запросы)", "0%", "4%", "+4", "Эксперименты + A/B-тесты"],
]
t = Table(budget, colWidths=[55*mm, 14*mm, 14*mm, 20*mm, 68*mm])
ts = std_table_style()
ts.add("ALIGN", (1, 1), (3, -1), "CENTER")
t.setStyle(ts)
story.append(t)
...
comp = [
    ["Игрок", "Тарифы", "AI?", "Боты?", "152-ФЗ?", "Слабое место"],
    ["Bitrix24", "Free / 2 990 / 7 990", "Да", "Да", "Да", "82% SMB, перегружен"],
    ["Tilda", "Free / 1 490 / 6 990", "Только текст", "Нет", "Нет", "+33% год к году, риск"],
    ...
    ["Yandex AI Studio", "По запросу", "Да (DeepSeek)", "Нет", "Да", "Партнёрство (500 М)"],
...
roadmap = [
    ["Период", "Этап", "Что делаем", "Целевой показатель"],
    ["Май-июнь 2026", "Подготовка", "Демо, страницы /about /docs, бета 100 человек, 3-дн пробник", "100 бета-пользователей + 3 кейса"],
    ["Июль 2026", "Бета", "Сбор отзывов, статьи на vc.ru/Habr, лендинг YandexGPT", "5-10 публикаций"],
    ...
    ["Январь-февраль 2027", "Стабилизация", "Удержание клиентов, A/B-тесты лендингов", "1 200 платящих"],
    ["Март-апрель 2027", "Пик 2 (B2B)", "Расширение E/F/G групп запросов", "1 670 платящих, выручка 3M+/мес"],
]
t = Table(roadmap, colWidths=[34*mm, 22*mm, 70*mm, 45*mm])
ts = std_table_style()
ts.add("VALIGN", (0, 0), (-1, -1), "TOP")
ts.add("FONTNAME", (0, 1), (0, -1), BOLD)
ts.add("FONTNAME", (1, 1), (1, -1), BOLD)
...
risks = [
    ["#", "Риск", "Серьёзность", "Что делаем чтобы снизить"],
    ...
    ["3", "Bitrix24 доминирует (82% SMB)", "СРЕД", "Фокус на простоту + SMB до 50 чел., не конкурируем фронтально"],
    ["4", "Разрыв между ценой привлечения и выручкой", "ВЫСОК", "3-дн пробник снижает CAC в 9 раз"],
    ...
    ["7", "Замедление SMB-рынка в 2026", "СРЕД", "Lean расходы + только подтверждённые GT-тренды"],
...
ts.add("FONTNAME", (1, 1), (1, -1), BOLD)
ts.add("ALIGN", (2, 1), (2, -1), "CENTER")
ts.add("FONTNAME", (2, 1), (2, -1), BOLD)
# Color severity
for i, r in enumerate(risks[1:], start=1):
    if r[2] == "КРИТ":
        ts.add("TEXTCOLOR", (2, i), (2, i), WARN)
        ts.add("BACKGROUND", (2, i), (2, i), WARN_BG)
    elif r[2] == "ВЫСОК":
        ts.add("TEXTCOLOR", (2, i), (2, i), ORANGE)
        ts.add("BACKGROUND", (2, i), (2, i), ORANGE_BG)
    else:
        ts.add("TEXTCOLOR", (2, i), (2, i), INK_MUTED)
...
sem_actions = [
    ...
    "Создать лендинг /yandexgpt-dlya-biznesa (растущий тренд)",
]
