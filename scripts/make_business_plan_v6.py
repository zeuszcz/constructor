...
def tokens_per_client(month):
    """AI-токены на одного клиента ₽/мес. Растёт с эволюцией смеси тарифов.
    Y1 (M1-M12): Lite-доминантный микс. На 1 клиента уходит 120-220 ₽ AI-кошелька.
    Y2 (M13-M24): рост Starter/Pro. 220-290 ₽.
    Y3 (M25-M36): зрелый микс с Enterprise. 290-360 ₽.
    Источник цифр: тарифная сетка раздел 6.2 (AI-кошелёк в каждом тарифе) × % распределения."""
    if month <= 12:
        return 120 + (220 - 120) * (month - 1) / 11
    elif month <= 24:
        return 220 + (290 - 220) * (month - 12) / 12
    else:
        return 290 + (360 - 290) * (month - 24) / 12

def our_infra(paying_count):
    """Наша инфраструктура (control plane): API-серверы, БД PostgreSQL,
    Redis, S3-бэкапы, мониторинг, CDN, защита от DDoS.
    Ступенчатая по числу клиентов — на каждом пороге нужно укрепление инфры."""
    if paying_count == 0: return 8000        # M1 — staging + dev VPS + минимум
    if paying_count <= 50: return 12000       # 1 VPS + базовое
    if paying_count <= 150: return 18000      # + managed PG
    if paying_count <= 350: return 32000      # + read-replica + S3
    if paying_count <= 700: return 55000      # + Redis + мониторинг
    if paying_count <= 1200: return 85000     # + кластер + DR
    if paying_count <= 2000: return 130000    # + observability
    if paying_count <= 3500: return 200000    # + multi-region backups
    if paying_count <= 5500: return 290000    # + CDN + DDoS premium
    if paying_count <= 8000: return 400000    # + on-call SRE
    if paying_count <= 11000: return 550000   # + dedicated cluster
    return 700000                              # 11k+ — enterprise-grade

def backoffice_per_client(month):
    """Бэк-офис на одного клиента: VPS клиентского сайта + его домен + комиссия ЮKassa.
    Растёт с эволюцией микса (Pro-клиенты получают мощнее VPS).
    Y1: 220 ₽/клиент (Lite 100+25 + Starter 250+50 средневзв.)
    Y2: 320 ₽/клиент (больше Pro 500+100)
    Y3: 400 ₽/клиент (Enterprise 1500+250)."""
    if month <= 12:
        return 220 + (320 - 220) * (month - 1) / 11
    elif month <= 24:
        return 320 + (400 - 320) * (month - 12) / 12
    else:
        return 400 + (470 - 400) * (month - 24) / 12

def operations_fix(month):
    """Постоянные операционные расходы: банк, почта, юр.техника, мелочь."""
    if month <= 6: return 5000
    if month <= 12: return 15000
    if month <= 24: return 25000
    return 40000
...
for i in range(36):
    ...
    # Реальные расходы: токены × N + наша инфра + бэкофис × N + операционка
    tokens = tokens_per_client(m) * p
    infra = our_infra(p)
    tokens_infra = tokens + infra  # для отображения в таблице
    backoffice = backoffice_per_client(m) * p + operations_fix(m)
    mkt = marketing[i]
    trial = trial_cost(m, p)
    total_opex = salaries + tokens_infra + backoffice + mkt + trial
    profit = mrr - total_opex
    cum += profit
    month_data.append({
        'm': m, 'paying': p, 'arpu': a, 'mrr': mrr,
        'sal': salaries, 'tokens': tokens, 'infra': infra, 'ti': tokens_infra,
        'bo': backoffice, 'mkt': mkt, 'trial': trial,
...
struct_rows = [
    ("Зарплаты команде", "30% от выручки\n(минимум 30 000 ₽/мес)",
     "Юрист + основатели + первые наёмные сотрудники. В первые 4 месяца включается минимум 30 К — это юрист (~20 К) + базовое самообеспечение основателя. С M5 30% от выручки уже превышает 30 К — модель автоматически растёт зарплатный фонд по мере роста выручки."),
    ("AI-токены (реальная цена за токены)", "Per-client × N клиентов",
     "Платежи Anthropic / OpenAI / Yandex / DeepSeek за токены. Цена = средневзв. AI-кошелёк на клиента × число платящих. Y1: 120-220 ₽/клиент (Lite-доминантный), Y2: 220-290 ₽, Y3: 290-360 ₽ (зрелый микс с Enterprise)."),
    ("Наша инфраструктура (control plane)", "Ступенчато по числу клиентов",
     "API-серверы, managed PostgreSQL, Redis, S3-бэкапы, мониторинг, CDN, защита от DDoS. Скачкообразный рост на порогах: 0-50 клиентов = 8-12 К; 350 = 32 К; 1200 = 85 К; 3500 = 200 К; 8000 = 400 К; 11000+ = 700 К. Каждый порог = модернизация одного из компонентов."),
    ("Бэкофис (VPS клиентам + домены + ЮKassa)", "Per-client × N + 5-40 К фикс",
     "На 1 клиента: VPS его сайта (100-1500 ₽ в зависимости от тарифа) + домен (25-250 ₽) + комиссия ЮKassa (3-3.5% от его платежа). Плюс операционка-фикс: банк, почта, юр.техника. Y1: 220 ₽/клиент + 15 К фикс. Y3: 400 ₽/клиент + 40 К."),
    ("Маркетинг и реклама", "Фиксированный бюджет",
     "100 К → 500 К ₽/мес. Растёт по сезонам (4 квартал пик, 3 квартал подготовка). Не привязан к выручке — это инвестиция в рост."),
    ("Расходы на пробник", "Фиксированно из плана",
     "20 К → 90 К ₽/мес. Зависит от объёма пробников. На 1 пробного пользователя ~130 ₽ (50 ₽ VPS + 80 ₽ AI-токены). Окупается 6.3× в первый же месяц через конверсию 20% в платящего."),
]
struct = [["Категория", "Формула расчёта", "Что включает и откуда цифры"]]
for cat, formula, what in struct_rows:
    struct.append([cp(cat, bold=True), cp(formula), cp(what)])
t = Table(struct, colWidths=[50*mm, 40*mm, 81*mm])
ts = std_table_style()
ts.add("VALIGN", (0, 0), (-1, -1), "TOP")
t.setStyle(ts)
story.append(t)
...
# Что изменилось vs процентная модель
story.append(Paragraph(
    "<b>Почему отказались от модели «токены+инфра = 30% выручки»:</b> "
    "процент от выручки даёт переоценку расходов по мере роста ARPU. Реальная цена токенов на клиента "
    "почти не зависит от того, на каком тарифе он сидит — Lite-клиент тратит 120 ₽ на AI-кошелёк, "
    "Enterprise тратит 1200 ₽, но средневзвешенно за весь Y1 = 150-200 ₽. Это значит при росте ARPU "
    "выручка растёт быстрее себестоимости, и реальная маржа выходит выше расчётной. По нашим оценкам "
    "разница: на M12 — старая модель 30% × 3.1 М = 927 К расходов, реальная ≈ 474 К → "
    "<b>+453 К прибыли в месяц</b>. Эта дельта уходит в накопленные деньги на счёте.",
    P
))
story.append(Spacer(1, 4*mm))

# Конкретный пример расчёта на M5 и M12
example_rows = [
    ("Месяц", "Платящих", "Формула AI-токены", "Формула инфра", "Формула бэкофис"),
    ("M5", "165", "165 × 156 ₽ ≈ 25 700 ₽",
     "(150-350 порог) → 32 000 ₽", "165 × 257 ₽ + 5 000 = 47 400 ₽"),
    ("M12", "1 670", "1 670 × 220 ₽ ≈ 367 400 ₽",
     "(1200-2000 порог) → 130 000 ₽", "1 670 × 320 ₽ + 15 000 = 549 400 ₽"),
    ("M24", "6 250", "6 250 × 290 ₽ ≈ 1 812 500 ₽",
     "(5500-8000 порог) → 400 000 ₽", "6 250 × 400 ₽ + 25 000 = 2 525 000 ₽"),
    ("M36", "11 900", "11 900 × 360 ₽ ≈ 4 284 000 ₽",
     "(11000+ порог) → 700 000 ₽", "11 900 × 470 ₽ + 40 000 = 5 633 000 ₽"),
]
example = [example_rows[0]]
for m, p, t1, t2, t3 in example_rows[1:]:
    example.append([cp(m, bold=True, center=True), cp(p, center=True), cp(t1, small=True),
                    cp(t2, small=True), cp(t3, small=True)])
t = Table(example, colWidths=[15*mm, 25*mm, 50*mm, 35*mm, 46*mm])
ts = std_table_style()
ts.add("ALIGN", (1, 1), (1, -1), "CENTER")
ts.add("VALIGN", (0, 0), (-1, -1), "MIDDLE")
t.setStyle(ts)
...
story.append(footer())
story.append(PageBreak())
page_num_counter[0] += 0  # Дополнительная страница в счётчике

page_title("9", "Финансовая модель (продолжение) — допущения по росту")
...
page_title("10", "Финансовая модель — Год 1 (май 2026 - апрель 2027)",
           "Помесячный план первого года с реальной стоимостью токенов и инфраструктуры. Самый большой минус ~−526 К ₽ на M7. Точка плюса операц. прибыли на M8. Возврат вложенных денег на M10.")
...
page_title("11", "Финансовая модель — Год 2 (май 2027 - апрель 2028)",
           "Второй год — выход на стабильную прибыль с реальной экономикой. К M24: 6 250 платящих, выручка 20 М/мес, накопленные деньги ~64 М ₽.")
...
page_title("12", "Финансовая модель — Год 3 (май 2028 - апрель 2029)",
           "Третий год — зрелая прибыль с реальной экономикой. К M36: 11 900 платящих, выручка 49 М/мес, годовая выручка ~580 М ₽, накопленные деньги ~262 М ₽.")
...
page_title("13", "Сводка финансовой модели за 36 месяцев",
           "Ключевые цифры и точки роста с реальной стоимостью токенов и инфры. Главный результат: на 36 мес — годовая выручка ~580 М ₽, накопленная прибыль ~262 М ₽, без внешних инвестиций.")
