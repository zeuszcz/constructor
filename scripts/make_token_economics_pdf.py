...
key_nums = [
    ["M1", "M8", "M11", "16.3 М ₽"],
    ["разработка завершена", "первый плюсовой месяц", "вернули вложенное", "выручка/мес"],
]
...
page_title(
    ...
    "Месяц 1 — разработка продукта (только разработчик + юрист). Месяц 2 — запуск и реклама сразу. Можно запуститься без инвесторов: максимум потеряем ~2 М ₽.",
)
...
main_pts = [
    ...
    "Базовые расходы компании — <b>330 К/мес</b> (юрист 30 + разработка 300). С месяца 2 добавляется маркетинг 100→250 К/мес.",
    "Месяц 1 — только разработка. Месяц 2 — запуск + реклама сразу. Прибыль покроет расходы на месяце 8 при ~720 платящих.",
    "За 18 месяцев модель выходит на <b>16.3 М ₽/мес выручки</b> (~196 М в год) и <b>+21.8 М ₽ на счёте</b>.",
