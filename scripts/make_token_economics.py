...
# Платящие клиенты — пересчитано под маркетинговую реальность 2026:
# M2-M6 замедленный разгон, с M7 догоняет исходный план (бренд + первые кейсы)
paying = [
    0,                                       # M1 тихий запуск
    10, 30, 75, 165,                         # M2-M5 (маркет.реальность: 8-12, 25-35, 65-85, 150-180)
    300, 480, 660, 870,                      # M6-M9 (M7+ начинает догонять)
...
# Средний доход с клиента — растёт от 990 → 4160 по мере зрелости микса
arpu = [
    0,
    990, 990, 1100, 1200,
    1400, 1700, 2000, 2300,
...
# ====================================================================== #
ws = wb.create_sheet("16. Маркетинг 2026 (clean)")
write_title(
    ws,
    1,
    "Семантическое ядро · 63 ЧИСТЫХ запроса · Wordstat 2026",
    "Только запросы с явным buying intent: vertical / AI-specific / pain-driven / research. Грязные (бренд+generic+freebie) исключены — экономят бюджет в 2-3× через downstream conversion.",
)

# Keyword data — ONLY CLEAN: (запрос, категория, показов/мес, CPC ₽ 2026, ценность)
keywords_2026 = [
    # A. Прямой коммерческий с buying intent (5)
    ("сайт под ключ", "A. Прямой intent", 18000, 135, 4),
    ("лендинг создать", "A. Прямой intent", 15000, 120, 4),
    ("сайт самостоятельно", "A. Прямой intent", 8000, 80, 4),
    ("одностраничный сайт", "A. Прямой intent", 5000, 90, 4),
    ("сайт без программиста", "A. Прямой intent", 1500, 65, 5),
    # B. AI (9)
    ...
    # C. Research only (4)
    ("wix или tilda", "C. Research", 1500, 65, 5),
    ("tilda аналог", "C. Research", 1100, 75, 5),
    ("конструктор сайтов сравнение", "C. Research", 800, 80, 4),
    ("замена tilda", "C. Research", 600, 70, 5),
    # D. Кафе
    ...
    # E. Магазин (без freebie 'магазина бесплатно')
...
# Headers
write_head(ws, 4, ["#", "Запрос", "Категория", "Показов/мес 2026", "Цена клика ₽ 2026", "Ценность 1-5"])
...
scenarios = [
    ("А. Cold start M2 (clean targeting)", 85, 0.04, 0.06),
    ("Б. Базовая оптимизация M3", 75, 0.05, 0.07),
    ("В. Vertical landing pages M4", 65, 0.06, 0.08),
    ("Г. Зрелая воронка M5+", 60, 0.07, 0.09),
    ("Д. Идеал M6+ (LTV/CAC 2.4)", 55, 0.08, 0.10),
]
...
ramp = [
    ...
    (2, "M2 Cold start (clean targeting)", 85, 0.040, 0.06),
    (3, "M3 Первая оптимизация", 75, 0.050, 0.07),
    (4, "M4 Vertical landing pages", 65, 0.060, 0.08),
    (5, "M5 Retargeting + узнаваемость", 60, 0.070, 0.09),
    (6, "M6 Зрелая воронка", 55, 0.080, 0.10),
]
...
check_data = [
    ("M1", 0, 0, "0", "—"),
    ("M2", 30, 3, "−27", "Multi-channel + soft-launch buzz → 8-12"),
    ("M3", 80, 5, "−75", "VC.ru/Habr статья + первые кейсы → 25-35"),
    ("M4", 150, 7, "−143", "Vertical lendings + B2B sales → 65-85"),
    ("M5", 250, 11, "−240", "Retargeting + узнаваемость → 150-180"),
    ("M6", 380, 15, "−365", "Зрелая воронка → 280-320"),
    ("M7+", "540+", "20-30", "догоняет", "Бренд + кейсы + органика"),
]
...
ws.cell(row=note_row, column=1, value=(
    "ВЫВОД: 100К ₽ ТОЛЬКО на ЧИСТЫХ запросах в Direct в первый месяц = 3 платящих "
    "(vs 1 на грязном ядре). Грязные запросы (тильда / создать сайт / сайт бесплатно) — "
    "это freebie hunters и loyalty конкурентов, downstream conversion 0.5-1%. На чистых "
    "vertical / AI / pain ключах CV лендинга +60% и Trial→Paid +20%. Финплан М2=30 требует "
    "мульти-канал (Direct 60К + VK 25К + Telegram 15К → 8-12) + buzz (VC.ru → +14)."
)).font = HEAD
